...
**到达过程**：兼容 legacy `arrival_model/min_inter_arrival/max_inter_arrival`，并支持统一 `arrival_process`（`fixed/uniform/poisson/one_shot/custom`）；`custom` 通过注册生成器扩展（内置 `constant_interval/uniform_interval/poisson_rate/sequence`）。
...
### 4.8 metrics（指标统计）
**职责**：消费事件流并生成统计指标。
**审计规则**：除资源配对/死锁外，新增协议一致性规则（`pip_priority_chain_consistency`、`pcp_ceiling_transition_consistency`），并输出 `rule_version/evidence/protocol_proof_assets` 审计证据字段。
...
### 4.10 cli（命令入口）
**职责**：提供 `validate/run/ui/batch-run/compare/inspect-model/migrate-config/plan-static/analyze-wcrt/export-os-config` 子命令，导出事件、指标、语义关系、统一计划工件与批量汇总。

**工程化扩展**：性能基线脚本 `scripts/perf_baseline.py` 作为 CI 门禁输入。

**统一计划工件**：`plan-static` 输出 `spec_fingerprint`、`semantic_fingerprint`、`planning_context`、`coverage_summary`、`runtime_static_windows`；`run/analyze-wcrt/export-os-config` 在消费 `--plan-json` 时默认做双指纹校验，`--allow-plan-mismatch` 是唯一显式放行入口，`--strict-plan-match` 仅保留兼容语义。
...
5. 输出指标/报告
6. planning 基于规范化语义投影输出统一计划工件，供 `analyze-wcrt` / `export-os-config` / `run --plan-json` 共享
