...
> ⚠️ 历史快照提示（2026-02-23）：文中 `211 tests` 等数字属于 2026-02-18 首轮审查快照，不代表当前主干。当前基线请以 `docs/11` 与 `scripts/quality_snapshot.py` 生成结果为准（当前本地快照：`236 passed`，覆盖率 `87.10%`）。
...
- 现象：历史版本存在测试数快照漂移。
- 证据：首轮快照为 211 tests、总覆盖率 87%；2026-02-23 本地快照为 236 passed、总覆盖率 87.10%。
