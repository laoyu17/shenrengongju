...
### 2.2 运行验证
- 全量测试：`python -m pytest -q`（117 tests 通过）
- 覆盖率：`python -m pytest --cov=rtos_sim --cov-report=term-missing -q`（TOTAL 86%）
...
| M-02 | 处理器类型核心数语义一致 | `rtos_sim/model/spec.py:129` | 未实现 | 未校验 `processor_types.core_count` 与 `cores` 数量一致 |
...
| M-07 | 任务/子任务/分段资源访问关系集合 | `rtos_sim/model/spec.py:217`、`rtos_sim/core/engine.py:335` | 部分实现 | 关系可推导但无独立导出/分析对象 |
| M-08 | 周期/偶发/零星（随机）到达模型 | `rtos_sim/model/spec.py:72`、`rtos_sim/core/engine.py:677` | 部分实现 | 周期/最小间隔支持；随机零星未建模 |
...
| M-12 | 大规模任务集（典型 >=1000） | `scripts/perf_baseline.py:122` | 部分实现 | 基线默认仍是 100/300；1000 仅抽样验证，未纳入门禁 |

## 4. 已验证 Bug（可复现）

### B-01（高）平台模型语义缺陷：`core_count` 不参与一致性校验
- 现象：配置中 `processor_types.core_count=4` 且仅声明 1 个 core，`validate` 仍通过。
- 证据：`rtos_sim/model/spec.py:129` 仅校验 ID 唯一与 type 引用，不校验数量一致。
...
### B-02（中）批量实验部分失败时 CLI 仍返回成功码
- 现象：`batch-run` 中存在失败用例（`failed>0`）时，命令仍返回 0。
- 证据：`rtos_sim/cli/main.py:175` 至 `rtos_sim/cli/main.py:193` 固定 `return 0`。
...
- 现象：`docs/11` 声称“103 tests / 覆盖率总 87%”。
- 证据：`docs/11-实施现状问题与Sprint规划.md:61`、`docs/11-实施现状问题与Sprint规划.md:62`；实测为 117 tests、总覆盖率 86%。
...
- 证据：`TaskGraphSpec` 仅有 `period/min_inter_arrival`，无“零星随机到达分布”字段（`rtos_sim/model/spec.py:72`）。
- 风险：研究场景中“偶发/零星随机性”实验不可直接表达。
...
### R-03（中）死锁检测链路缺失
- 证据：SRS 将其作为风险（`docs/04-详细版SRS.md:114`），但运行时与审计规则未见 deadlock 检测项（`rtos_sim/analysis/audit.py:16`）。
- 风险：资源绑定 + 多锁顺序下可能出现“无报错卡住”场景。
