...
> ⚠️ 历史快照提示（2026-02-23）：文中 `211 tests` 等数字属于 2026-02-18 首轮审查快照，不代表当前主干。当前基线请以 `docs/11` 与 `scripts/quality_snapshot.py` 生成结果为准（当前本地快照：`234 passed`，覆盖率 `87.24%`）。
...
- 未实现/缺口：研究级可证明资产（形式化证明/边界报告）、更丰富自定义到达分布模板、多方案实验报告模板。
...
### 2.2 运行验证
- 全量测试：`python -m pytest --maxfail=1`（211 tests 通过）
- 覆盖率：`python -m pytest --cov=rtos_sim --cov-report=term-missing -q`（TOTAL 87%，其中 `cli/main` 100%）
- 性能抽样（本地）：300 tasks ≈ 321.74ms；1000 tasks ≈ 1581.67ms（脚本内 `_run_case`）

### 2.3 2026-02-22 快照补充
- 混合实时到达模型已支持统一 `arrival_process`（`fixed/uniform/poisson/one_shot/custom`），随机零星场景具备基础表达能力（后续侧重扩展分布模板库）。
- CI 性能门禁现状：PR 路径 100/300 软门禁，nightly 路径 1000 非阻断趋势任务 + delta 摘要。
- 审计报告已新增 `compliance_profiles`（`engineering_v1/research_v1`），研究闭环具备机器可判定基础。
- 研究级差距仍在“协议可证明性资产深度与模板化实验报告”，该结论与首轮审查保持一致。
...
| M-08 | 周期/偶发/零星（随机）到达模型 | `rtos_sim/model/spec.py:72`、`rtos_sim/core/engine.py:677` | 部分实现 | 已支持 `fixed/uniform/poisson/one_shot/custom`；仍缺分布模板库与报告模板 |
...
- 现象：历史版本存在测试数快照漂移。
- 证据：首轮快照为 211 tests、总覆盖率 87%；2026-02-23 本地快照为 234 passed、总覆盖率 87.24%。
- 影响：管理与排期依据失真，可能误判质量趋势。
- 建议修复：通过 `scripts/quality_snapshot.py` 生成带时间戳与 `git_sha` 的质量快照，避免手写过期。

## 5. 设计/架构风险（未必已复现）

### R-01（高）混合实时到达模型未完全覆盖 docx 语义
- 证据：已支持 `arrival_process.fixed/uniform/poisson/one_shot/custom`，`custom` 具备注册扩展入口（`rtos_sim/model/spec.py:72`）。
- 风险：研究场景可表达常见随机到达，但“分布模板库 + 报告模板”尚未体系化沉淀。
