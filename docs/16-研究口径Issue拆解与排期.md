...
- 全量测试：主线事实以 `artifacts/quality/quality-snapshot.json` 为准（当前基线 `484 passed`）。
- 覆盖率：主线事实以 `artifacts/quality/quality-snapshot.json` 为准（当前基线 `90.04%`，`line_rate=90.03965692151618`）。
