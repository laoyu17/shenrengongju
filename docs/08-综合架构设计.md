...
- 边界：不包含调度算法与仿真控制逻辑。
- 到达过程：支持 legacy 到达字段与统一 `arrival_process`（`fixed/uniform/poisson/one_shot`）。
...
- 边界：只消费事件流，不修改仿真状态。
- 审计补充：支持等待图死锁检测与协议一致性规则（`pip_priority_chain_consistency`、`pcp_ceiling_transition_consistency`）。
