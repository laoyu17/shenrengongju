...
rtos_sim/
  ...
  arrival/       # 到达过程生成器与注册机制
  schedulers/    # 调度策略与注册机制
  protocols/     # 资源访问协议（mutex/pip/pcp）
  etm/           # 执行时间模型（constant/table-based）
  overheads/     # 开销模型（切换/迁移/调度开销）
  metrics/       # 指标计算与可视化数据
  planning/      # 离线规划 / WCRT / 规范化语义层
  analysis/      # 审计 / 对比 / 模型关系 / 研究报告
  io/            # 配置加载/校验/实验批跑
  cli/           # CLI 真相源入口（validate/run/ui/batch-run/compare/inspect-model/migrate-config/plan-static/analyze-wcrt/benchmark-sched-rate/export-os-config）
...
- 边界：不包含调度算法与仿真控制逻辑。
- 到达过程：支持 legacy 到达字段与统一 `arrival_process`（`fixed/uniform/poisson/one_shot/custom`）；`custom` 由注册生成器驱动（内置 `constant_interval/uniform_interval/poisson_rate/sequence/periodic_jitter/burst_sequence`）。
- 配套子系统：`arrival/` 负责到达生成器注册、参数解析与最小间隔/随机性语义复用，供 `model/core/planning` 共用。
...
- 边界：只消费事件流，不修改仿真状态。
- 审计补充：支持等待图死锁检测与协议一致性规则（`pip_priority_chain_consistency`、`pcp_ceiling_transition_consistency`），并输出 `rule_version/evidence/protocol_proof_assets` 追溯字段。
- 配套子系统：`analysis/` 负责审计报告、Compare 报告、模型关系报告与 Research Report 产物编排。
...
### 3.10 planning/
- 职责：离线规划、WCRT 分析、统一计划工件与规范化语义投影。
- 边界：负责 `ModelSpec -> NormalizedExecutionModel -> PlanningProblem` 的桥接，并向 CLI/UI/runtime 导出统一工件（`spec_fingerprint` / `semantic_fingerprint` / `runtime_static_windows`）。

### 3.11 cli/
- 职责：统一命令入口，提供 `validate/run/ui/batch-run/compare/inspect-model/migrate-config/plan-static/analyze-wcrt/benchmark-sched-rate/export-os-config` 命令。
...
6. planning 复用规范化语义投影生成统一计划工件，供 `analyze-wcrt` / `export-os-config` / `run --plan-json` 共享
