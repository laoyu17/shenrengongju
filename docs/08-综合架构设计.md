...
- 边界：不包含调度算法与仿真控制逻辑。
- 到达过程：支持 legacy 到达字段与统一 `arrival_process`（`fixed/uniform/poisson/one_shot/custom`）；`custom` 由注册生成器驱动（内置 `constant_interval/uniform_interval/poisson_rate/sequence`）。
...
### 3.11 cli/
- 职责：统一命令入口，提供 `validate/run/ui/batch-run/compare/inspect-model/migrate-config/plan-static/analyze-wcrt/export-os-config` 命令。
