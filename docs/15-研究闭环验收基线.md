...
- 模板报告口径：同一 rule 的失败项已聚合 `issue_count/sample_count/sample_event_ids`。
- 结论：研究闭环已具备“机读判定 + 反例集 + 模板报告 + CI 非阻断产物”完整执行链路。
...
- 在 CI Step Summary 中显式展示 `failed_rules` 与画像状态告警，降低“非阻断任务被忽略”风险
