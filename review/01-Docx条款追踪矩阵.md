...
| G-04 可调度率提升 30%（口径） | `review/docx/baseline_design_report_extracted.md:87` | `rtos_sim/cli/main.py:453`；`scripts/benchmark_sched_rate.py:1` | `tests/test_sched_rate_benchmark_script.py:22` | `review/runtime/logs/11_benchmark_script_strict.log:1`（`macro_uplift=0.833333333`） | 部分实现 |
| G-05 图形化输入/输出界面 | `review/docx/baseline_design_report_extracted.md:88` | `rtos_sim/ui/app.py:636`（Planning页）`rtos_sim/ui/app.py:668`（结果Tab） | `tests/test_ui_gantt.py:242` | `python -m pytest -q` 全绿（UI用例含该场景） | 已实现 |
| G-06 平台参数与任务模型参数配置 | `review/docx/baseline_design_report_extracted.md:89` | `rtos_sim/ui/config_doc.py:269`（planning 默认与补丁）；`rtos_sim/io/schema.py:227` | `tests/test_cli_planning_commands.py:405`（迁移补全 planning） | `review/runtime/logs/13_migrate_at01.log:1` | 已实现 |
| G-07 运行时动态仿真与状态展示 | `review/docx/baseline_design_report_extracted.md:90` | `rtos_sim/ui/controllers/timeline_controller.py:53`~`102`（Released/Ready/Executing/Blocked） | `tests/test_ui_gantt.py:283`；`tests/ui/test_timeline_controller.py:195` | `python -m pytest -q` 全绿（状态流测试覆盖） | 已实现 |
| G-08 任务调度参数输出接口 | `review/docx/baseline_design_report_extracted.md:91` | `rtos_sim/cli/handlers_planning.py:237`；`rtos_sim/api.py:260` | `tests/test_cli_planning_commands.py:134` | `review/runtime/logs/06_export_os_at06_strict_match.log:1` | 已实现 |
